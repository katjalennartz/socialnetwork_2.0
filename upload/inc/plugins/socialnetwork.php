...
$plugins->add_hook("build_forumbits_forum", "socialnetwork_getglobals");
function socialnetwork_getglobals(&$forum)
{
    global $mybb, $db, $templates, $parser, $lang, $sn_newsfeedFriend, $sn_newsfeedAll, $sn_page, $last_post, $userinfo;
    ...
    if ($db->num_rows($get_lastpost) > 0) {
        ...
        if (!empty($postImg)) {
            if ($mybb->settings['socialnetwork_images_guests'] == 0) {
                $postImgFilename = "";
                if ($mybb->settings['socialnetwork_images_guests_default'] != "") {
                    $postImgFilename = $mybb->settings['socialnetwork_images_guests_default'];
    ...
    $showingame = $mybb->settings['socialnetwork_ingame'];
    $ingamefid = $mybb->settings['socialnetwork_ingamefid'];
    // echo $ingamefid;
    if ($showingame && $forum['fid'] == $ingamefid) {
        $forum['socialnetwork'] = eval($templates->render('socialnetwork_forumbit_ingame'));
    }

    eval("\$socialnetwork_global_ingame .= \"" . $templates->get('socialnetwork_global_ingame') . "\";");
...
// /**
//  * Function for showing above Ingame 
//  * 
//  */
// $plugins->add_hook("build_forumbits_forum", "socialnetwork_forumingame");
// function socialnetwork_forumingame(&$forum)
// {
//     global $mybb, $db, $templates, $parser, $lang, $sn_newsfeedFriend, $sn_newsfeedAll, $sn_page, $last_post, $userinfo;
...
{$socialnetwork_ucp_pmAlert}
    ...
    $template[27] = array(
        "title" => 'socialnetwork_forumbit_ingame',
        "template" => '
        <div class="sn_userlist_container__item sn_userlistbit">
        <div class="sn_userlistbit__item sn_userlistbit__avatar">{$last_post[\\\'sn_avatar\\\']}</div>
        <div class="sn_userlistbit__item sn_userlistbit__post">{$last_post[\\\'sn_social_post\\\']}</div>
        <div class="sn_userlistbit__item sn_userlistbit__link">{$last_post[\\\'linktopost\\\']}</div>
        </div>
        ',
        "sid" => "-2",
        "version" => "1.0",
    ...
    $template[28] = array(
        "title" => 'socialnetwork_global_ingame',
        "template" => '
        <div class="sn_userlist_container__item sn_userlistbit">
        <div class="sn_userlistbit__item sn_userlistbit__avatar">{$last_post[\\\'sn_avatar\\\']}</div>
        <div class="sn_userlistbit__item sn_userlistbit__post">{$last_post[\\\'sn_social_post\\\']}</div>
        <div class="sn_userlistbit__item sn_userlistbit__link">{$last_post[\\\'linktopost\\\']}</div>
        <div class="">{$sn_newsfeedFriend} - {$sn_newsfeedAll}</div>
        </div>
        ',
        "sid" => "-2",
        "version" => "1.0",
...
function socialnetwork_add_settings($type = 'install')
{
    ...
    $setting_array = array(
        ...
        'socialnetwork_ingame' => array(
            'title' => $lang->socialnetwork_settings_ingame_tit,
            'description' => $lang->socialnetwork_settings_ingame_default,
            'optionscode' => 'yesno',
            'value' => 'no', // Default
            'disporder' => 17
        ),
        'socialnetwork_ingamefid' => array(
            'title' => $lang->socialnetwork_settings_ingamefid_tit,
            'description' => $lang->socialnetwork_settings_ingamefid_default,
